...
func returnAuth() *Auth0 {
	auth0Details := Auth0{Aud: os.Getenv("AUD"), Iss: os.Getenv("ISS")}
	fmt.Println(auth0Details)
	return &auth0Details
}
func main() {
	auth0Details := returnAuth()

	jwtMiddleware := jwtmiddleware.New(jwtmiddleware.Options{
		ValidationKeyGetter: func(token *jwt.Token) (interface{}, error) {
			// Verify 'aud' claim
			aud := auth0Details.Aud
			checkAud := token.Claims.(jwt.MapClaims).VerifyAudience(aud, false)
			if !checkAud {
				return token, errors.New("Invalid audience.")
			}
			// Verify 'iss' claim
			iss := auth0Details.Iss
...
func getPemCert(token *jwt.Token) (string, error) {
	cert := ""
	auth0Details := returnAuth()
	resp, err := http.Get(auth0Details.Iss + ".well-known/jwks.json")
